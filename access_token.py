...
def create_access_token_file():
  now = dt.datetime.now()
  igdb_access_url = "https://id.twitch.tv/oauth2/token?client_id=" + CLIENT_ID + "&client_secret=" + CLIENT_SECRET + "&grant_type=" + GRANT_TYPE
...
def check_access_token_expiration():
  ...
  exp_date = dt.datetime.strptime(acquired, dt_time_format) + dt.timedelta(seconds = expires)
...
def get_access_token():
  try:
    ...
    if os.path.exists(ACCESS_TOKEN_FILE) and check_access_token_expiration() >= TOKEN_BUFFER:
      ...
      print("Access token file does not exist or is no longer valid, creating/updating")
